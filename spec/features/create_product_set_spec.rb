...
describe "creating an product set", type: :feature, js: true do
  ...
  it "creates an product set" do
    ...
    fill_in('product_set_name', with: @name)
    page.execute_script('$("#product_set_scout_date").val("2016-06-11") ' )
    fill_in('product_set_location', with: @location)
    fill_in('product_set_notes', with: @notes)
